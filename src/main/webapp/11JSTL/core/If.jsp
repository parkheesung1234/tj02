...
속성정리
 ...
 -->
	...
	result : ${ result } <br />
	<!-- 
	JSTL의 if태그는 else구문이 별도로 없으므로 if문과의 반대의 조건을 만들어서
	두 개의 if태그를 사용한다.
	 -->
	...
	값에 대한 비교와 문자열에 대한 비교 모두를 할 수 있다. 여기서 사용한 eq는 
	Java의 compareTo()와 같이 사전순으로 문자열을 비교한다.
	 -->
	 <!-- 
	...
	result4 : ${ result4 } <br />
	<!-- 
	test에는 앞뒤에 공백이 하나라도 들어가면 무조건 false를 반환하게 된다. 따라서 주의해야한다.
	 -->
	...
	<c:if test="${not empty param.user }">
		...
		if 태그는 else구문이 별도로 없으므로 not을 통해 else와 같은 구문을 구성해야한다.
		-->
