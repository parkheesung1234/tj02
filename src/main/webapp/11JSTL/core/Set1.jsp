...
<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<!-- 
JSTL의 core태그를 사욯아기 위한 지시어 선언. prefix는 접두어, uri는 태그를 지원하는
예약어가 들어간다.
 -->
...
<!-- 
set태그 :
변수를 선언할 때 사용. JSP의 setAttirbute()와 동일한 기능을 한다.
4가지 영역에 새로운 속성을 추가한다.
var : 속성명(변수명). 즉, 영역에 저장될 이름
value : 속성에 저장될 값
scope : 4가지 영역명을 지정. 미 지정시 가장 좁은 page영역에 저장됨. 
target : set태그를 통해 생성된 자바빈즈의 이름을 지정함.
property : target으로 지정된 자바빈즈의 멤벼변수값을 지정한다.

 -->
	<!-- 변수 선언 : 여기ㅓ는 가장 좁은 page 영역에 저장된다. -->
	<!-- value에는 일반값 값, EL, 표현식을 모두 사용할 수 있다. 또한,
	태그 사이에 value를 삽입할 수도 있다. -->
	...
	<ul>
	 ...
	 더블쿼테이션이 겹쳐지는 경우가 발생하게 된다. 이 때는 value를 싱글 쿼테이션으로
	 감싸서 겹쳐지지 않게 처리해야한다. -->
