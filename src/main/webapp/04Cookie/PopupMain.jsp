...
<%
/*
해당 페이지에서는 가장 먼저 팝업창 오픈에 관련된 쿠키가 있는지 먼저 확인한 후
팝업창을 처리해야한다.
*/
//팝업창 오픈 여부 판단을 위한 변수
String popupMode = "on"; 
...
$(function() {
    ...
    $('#closeBtn').click(function(){
    	...
    	$('#popup').hide();
    	/*
    	하루동안 열지않음 체크박스에체크한 경우에는 value를 얻어와서 변수에 저장한다.
    	value는 1로 설정되어 있다.
    	*/
    	...
    	$.ajax({
    		...
    		*/
    		...
    		//콜벡데이터의 타입은 test형식으로 지정한다.
    		dataType : "text",
    		//요청성공 시 호출될 콜백함수
    		success : function(resData){
    			/*
    			콜백데이터에 지시어 때문에 남게되는 공백때문에 불필요한 새로고침이
    			일어날 수 있다. 따라서 공백을 제거해야 정상적으로 동작하게 된다.
    			*/
    			if(resData){
    				console.log('있다.');
    			}
    			...
    			/*
    			만약 콜벡데이터가 있다면 페이지를 새로고침한다. F5를 누르는것과 동일하다.
    			*/
...
<%
    ...
    for (int i = 1; i <= 10; i++) {
    	/*
    	변수 pupupMode의 초기값은 on이고, 만약 쿠키가 생성되었다면 off로
    	설정된다.
    	*/
...
즉, 쿠키의 상태에 따라 페이지가 동적으로 변경된다. 이와 같은 것을 "동적웹페이지"라고 한다.
*/
