...
//세션영역이 저장된 회원 아이디를 가져와서 문자열로 반환한다.
String sessionId = session.getAttribute("UserId").toString();
//로그인한 회원이 해당 게시물을 작성자인지 확인한다.
...
/*
글쓰기 페이지에서 제목과 내용이 입력되었는지 검증하여 JS코드
*/
...
action속성값을 반드시 수정해야한다. 만약 수정하지 않으면 
게시물이 추가되는 헤프닝이 생기게된다.
 -->
<form name="writeFrm" method="post" action="EditProcess.jsp"
    ...
    <input type="hidden" name="num"	value="<%= dto.getNum() %>" />
   	<!-- type속성은 text로 인식하기 때문에 개발할 때는 hidden사이에 스페이스나 (-)을 넣어 확인할수 있게 하는것도 좋다 -->
