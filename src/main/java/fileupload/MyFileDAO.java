...
/* 커넥션풀(DBCP)을 이용해서 오라클 DB에 연결한다. 설정 정보는 server.xml,
  connect.xml에 기술되어 있다. 이클립스는 복사본을 사용하게 되고, 원본은
		...
		public int insertFile(MyFileDTO dto) {
			...
			try {
				/* 인파라미터가 있는 동적 쿼리문으로 작성. 클라이언트가 폼에 
				  입력한 내용을 DTO로 저장 후 전달하면 해당 쿼리문의 인파라미터를
				  설정하게 된다. */
