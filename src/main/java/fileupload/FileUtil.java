...
public class FileUtil {
	...
	public static String uploadFile(HttpServletRequest req, String sDirectory) throws ServletException, IOException{
		...
		/*
		파일 첨부를 위한 <input> 태그의 name 속성값을 이용해서 Part객체를
		...
		*/
		...
		/*
		앞에서 split()한 결과중 인덱스1은 파일명이 된다. 여기서 더블쿼테이션을 제거하면
		순수한 파일명만 남는다. replace()를 통헤 제거한다. 이 때, 더블쿼테이션을 제거할 문자열로
		사용하기 위해 이스케이프시퀀스를 붙여줘야한다.
		*/
		String originalFileName = phArr[1].trim().replace("\"", "");
				
		/*
		전송된 파일이 있는 경우라면 디렉토리에 파일을 저장한다. 이 때 write() 메서드를 사용한다.
		File.separator
		운영체제(OS)마다 경로를 표시하는 기호가 다르므로 해당 OS에 맞는것을
		자동으로 기술해준다.
		*/
	...
	public static String renameFile(String sDirectory, String fileName) {
		...
		/*
		파일명에서 확장자를 잘라내기 위해 뒤에서부터 .이 있는 위치를 찾는다. 파일명에는 2개 이상의
		.을 사용할 수 있기 때문이다.
		*/
