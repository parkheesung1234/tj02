...
public class PostLogin extends HttpServlet{
   ...
   protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
      ...
      /* 서블릿이 직접 요청을 받으므로 application내장객체는 아래
        메서드를 통해 얻어와야 한다. */
      ServletContext application = this.getServletContext();
      //DAO객체 생성 및 회원인증 
      MemberDAO dao = new MemberDAO(application);
      MemberDTO dto = dao.getMemberDTO(id, pw);
      /* Java에서 JSON데이터를 생성하기 위한 확장 라이브러리 json-simple.jar를
        프로젝트에 설치한 후 아래 객체를 생성한다.
        JSON은 객체와 배열로 구분되는데 객체인 경우 JSONObject를
        사용하고, 배열의 경우 JSONArray를 사용한다. */
