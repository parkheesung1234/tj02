...
//커넥션풍르 통한 DB연결을 위한 클래스 상속
public class MVCBoardDAO extends DBConnPool {

	//기본생정자 (정의하지 않아도 자동으로 삽입된다.)
	public MVCBoardDAO() {
		super();
	}
	
	/*
	게시물의 갯수를 카운트한다. 검색어가 있는 경우 where절을 추가하여
	조건에 맞는 게시물을 카운트한다.
	*/
	public int selectCount(Map<String, Object> map) {
		...
		try {
			...
			/*
			count(*)는 항상 결과값이 있다. 조건에 맞는게 없으면 0, 아니면 1이상의
			값이 항상 반환한다.
			*/
			rs.next();
	...
	public List<MVCBoardDTO> selectListPage(Map<String,Object> map){
		...
		//mbcboard테이블을 대상으로 하므로 타입매개변수 확인 필요함.
		List<MVCBoardDTO> board = new Vector<MVCBoardDTO>();
		/*
		페이징 처리를 위한 서브쿼리문. 특정 페이지에 해당하는 rownum의 구간으로
		게시물을 인출한다.
		*/
		String query = " "
				+ "SELECT * FROM ( "
		...
		try {
			/*
			인파라미터가 있는 동적쿼리문이므로 prepared 객체 생성
			및 인파라미터 설정
			*/
	...
	public boolean confirmPassword(String pass, String idx) {
		...
		try {
			...
			rs.next();
			//열과가 0이라면 조건에 맞는 게시물이 없으므로 false저장
			if(rs.getInt(1)==0) {
				isCorr = false;
			}
		}
			...
			/*
			쿼리문 실행도중 예외가 발생되면 바로  catch절로 넣어오므로 이 경우에도
			false로 값을 설정해야한다.
			 */
			isCorr = false;
			e.printStackTrace();
