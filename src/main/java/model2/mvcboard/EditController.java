...
public class EditController extends HttpServlet {
	...
	/*
	수정 페이지로 진입하면 기존의 내용을 가져와서 쓰기폼에 세팅한다. 단순한
	페이지 이동이므로 get방식 요청이다. 
	 */
	@Override
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		//DTO객체를 request 영역에 저장한 후 포워드 한다.
	...
	게시판은 post 방식으로 전송되므로 doPoas()를 오버라이딩 한다.
	*/
	@Override
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		/*
		수정 내용을 매개변수에서 얻어옴
		수정을 위해 hidden박스에서 저장된 내용도 함께 받아온다. 게시물의
		...
		*/
		...
		if(originalFileName != "") {
			/*
			수정페이지에서 새롭게 등록된 파일이 있다면 기존 내용을 수정해야한다. 파일명의 이름을 변경한 후
