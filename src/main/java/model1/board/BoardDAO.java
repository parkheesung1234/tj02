...
public class BoardDAO extends JDBConnect{
	...
	public int selectCount(Map<String, Object>map) {
		...
		*/
		...
		try {
			...
			rs.next();
			//첫번째 컬럼(count 함수)의 값을 가져와서 변수에 저장한다.
	...
	순서를 보장해야 하므로 List 컬렉션을 사용한다.
	*/
	public List<BoardDTO> selectList(Map<String, Object>map){
		/*
		List계열의 컬렉션을 생성한다. 이 때 타입 매개변수는 board테이블을 대상으로
		하므로 BoardDTO로 설정한다.
		*/
		List<BoardDTO> bbs = new Vector<BoardDTO>();
		/*
		레코드 인출을 위한 select 쿼리문 작성. 최근 게시물이 상단에 출력되야 하므로
		일련번호의 내림차순으로 정렬한다.
		*/
	...
	public List<BoardDTO> selectListPage(Map<String, Object>map){
		...
		/*
		검색조건에 일치하는 게시물을 얻어온 후 각 페이지에 출력할 구간까지 설정한
		서브 쿼리문 작성
		*/
		...
		사용할 수 있다. 아래의 where절은 rNum>?과 같이 변경할 수 있다.
		*/
		query += "		ORDER BY num DESC "
			   + "		) Tb "
			   + " ) "
	...
	public int inserWrite(BoardDTO dto) {
		...
		{
			/*
			인파마리터가 있는 동적쿼리문으로 insert문을 작성한다.
	...
	//인수로 전달된 게시물의 일ㅇ련번호로 하나의 게시물을 인출한다.
	public BoardDTO selectView(String num) {
		...
		*/
		...
		try {
			...
			rs = psmt.executeQuery();
			/*
			일련번호는 중복되지 않으므로 단 한개으 ㅣ게시물만 인출하게 된다.
