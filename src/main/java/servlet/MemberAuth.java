...
public class MemberAuth extends HttpServlet {
	...
	/*
	플라이언트가 최초로 요청했을 때  서블릿 객체가 생성되는데 이때
	...
	*/
	...
	public void init() throws ServletException {
		...
		/*
		서블릿 내에서 application 내장객체를 얻어온다. 모델2 방식에서는 서블릿이
		먼저 요청을 받기 때문에 모델 1 방식과 같이 JSP에서 매개변수로 내장객체를 
		전달할 수 없다. 따라서 각 내장객체를 얻어올 수 있는 메서드가 존재한다.
		*/
		ServletContext application = this.getServletContext();
		
		
		//web.xml에 저장도니 컨텍스트 초기화 파라미터를 얻어온다.
		...
		*/
		...
		if(memberName != null) {
			...
			*/
			...
			/*
			회원인증에 실패한 경우에는 서블릿 초기화 파라미터와 비교해서 최고관리자안지 확인한다.
